classdef gmdistribution_lmm < classreg.learning.internal.DisallowVectorOps
...
%b random effects terms (size = num subjects x num gaussians x num dimensions)
        bhi = [];
...
%   the value of the NegativeLogLikelihood property.
%
%   See also GMDISTRIBUTION.
...
%   where  where NlogL is the value of the NegativeLogLikelihood property,
...
%   'ProbabilityTolerance' input for FITGMDIST.
   ...
   methods
       ...
       function ns = get.NumSubjects(this)
           ns = this.NSubjects;
       end
    ...
    methods
        function obj = gmdistribution_lmm(mu, Sigma, p, b, Sigma_eff)
        %GMDISTRIBUTION Create a Gaussian mixture model.
        %   GM = GMDISTRIBUTION(MU,SIGMA,P) creates a distributionl
        ...
        %   See also GMDISTRIBUTION, FITGMDIST.
            ...
            % mu - mu no longer represents the mean of each gaussian in the
            % mixture, but rather is beta in the formulation with LMM.
            % (where the true gaussian mu = beta + random effects.
            %
            % b - is a SxKxD matrix representing the random effect for each
            % subject S at each gaussian K.
            % 
            % Sigma_eff - is a DxD matrix representing the random effects
            % variance
            
            [s0,k0,d0] = size( b );
            if (k0 ~= k)
                error( message( 'stats:gmdistribution:RandEffIncorrectNumberofGaussians') );
            end
            if (d0 ~= k)
                error( message( 'stats:gmdistribution:RandEffIncorrectDimension') );
            end
            
            [ds1, ds2] = size( Sigma_eff );
            if ( ds1 ~= ds2 )
                error( message( 'stats:gmdistribution:NonSquareSigmaEff' ) );
            end
            if ( ds1 ~= d )
                error( message( 'stats:gmdistribution:SigmaEffWrongDimension' ) );
            end
            
            obj.Sigma_eff = Sigma_eff;
            obj.NSubjects = s0;
            obj.bhi = b;
